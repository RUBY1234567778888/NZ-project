...
#read data
variable_mapping <- read_excel("NZ_data_request_variables_explained.xlsx")

eligibility <- read.csv("Alleligibility-currentvalues_2024-10-29_0941.csv", stringsAsFactors = FALSE)
baseline <- read.csv("Baselineexport_2024-10-29_0941.csv", stringsAsFactors = FALSE)
microbiology <- read.csv("Microbiologyexport_2024-10-29_0941.csv", stringsAsFactors = FALSE)
discharge <- read.csv("Dischargeexport_2024-10-29_0941.csv", stringsAsFactors = FALSE)
registry <- read.csv("NZLongRegistryCRFExport_2024-10-29_0942.csv", stringsAsFactors = FALSE)
...
#Generated data
write.xlsx(randomised_data, "RandomisedData.xlsx", rowNames = FALSE)
registry_data <- registry_data %>%
  filter(!is.na(RegistryPatientID) & RegistryPatientID != "")
write.xlsx(registry_data, "RegistryData.xlsx", rowNames = FALSE)
```
...
organ_summary <- combined_organ %>%
  ...
  mutate(
     p_value = ifelse(p_value < 0.05, "<0.05", sprintf("%.3f", p_value)),
...
ggplot(randomised_data, aes(x = factor(Bas_APACHEScore))) +
  geom_bar(fill = "#66c2a5", alpha = 0.8) +
  labs(
    title    = "Figure8: APACHE Score Distribution in Randomised Group",
    subtitle = paste0(
      "Shapiro-Wilk: W = ", round(shapiro_test$statistic, 3),
      "   p = ", format.pval(shapiro_test$p.value, digits = 2)
    ),
    x = "APACHE II Score",
    y = "Count"
...
n_registry   <- nrow(registry_summary)
deaths_reg   <- sum(registry_summary$Death_Flag,    na.rm = TRUE)
surv_reg     <- n_registry - deaths_reg

n_randomised <- nrow(randomised_summary)
deaths_rnd   <- sum(randomised_summary$Death_Flag, na.rm = TRUE)
surv_rnd     <- n_randomised - deaths_rnd
...
metrics <- tibble(
  Metric    = c("Relative Risk (RR)", "Odds Ratio (OR)"),
  Estimate  = sprintf("%.2f",       c(RR_est,        OR_est)),
  CI_Low    = sprintf("%.2f",       c(RR_CI[1],      OR_CI[1])),
  CI_High   = sprintf("%.2f",       c(RR_CI[2],      OR_CI[2]))
)
print(rates)
print(metrics)
